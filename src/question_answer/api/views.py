    ...
    def get(self, request, pk):
        try:
            ...
            if len(question_answer) != 0:
                ...
                question_answer_serializer = GetSecretQuestionBasedOnParentIDSerializer(
                    question_answer, context=context, many=True)
...
class UpdateSecretQuestionBasedOnParentID(ListCreateAPIView):
    def put(self, request, pk):
        try:
            ...
            if QuestionAnswer.objects.filter(user=pk).exists():
                question_answer = QuestionAnswer.objects.filter(user=pk)
    ...
    def post(self, request):
        try:
            if User.objects.filter(id=request.data.get('parent_id', None)).exists():
                user_qs = User.objects.get(
                    id=request.data.get('parent_id', None))
