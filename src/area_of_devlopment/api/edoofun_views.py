...
class GetConceptListBasedOnSubjectID(ListCreateAPIView):
    def get(self, request, pk):
        try:
            ...
            if activity_qs:
                skill_qs_serializer = SkillConceptSerializer(
                    Skill.objects.filter(activity__in=activity_qs), many=True)
...
class GetSkillList(ListCreateAPIView):
    ...
    def get(self, request):
        try:
            ...
            if len(skill_qs) != 0:
                skill_qs_serializer = EdooFunSkillListSerializer(
                    skill_qs, many=True)
                print("skill_qs_serializer", skill_qs_serializer.data)
...
class GetConceptList(ListCreateAPIView):
    ...
    def get(self, request):
        try:
            ...
            if len(Concept_qs) != 0:
                Concept_qs_serializer = EdooFunConceptListSerializer(
                    Concept_qs, many=True)
...
class GetConceptDetailBasedOnConceptId(ListCreateAPIView):
    model = Concept
    filterset_class = ConceptFilter

    def get(self, request, pk):
        try:
            Concept_qs = Concept.objects.filter(id=pk)
            print("Concept_qs-----", Concept_qs)
            if len(Concept_qs) != 0:
                Concept_qs_serializer = EdooFunConceptListSerializer(
                    Concept_qs, many=True)
