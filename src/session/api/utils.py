    ...
    try:
        ...
        if request_data['calendar_type'] == 'school-calender':
            school_holiday_count = SchoolHoliday.objects.filter(
                holiday_from__lte=date_range_by_month['first_day'], holiday_till__gte=date_range_by_month['first_day'], school=request_data['school']).count()
            ...
            school_holiday_count = SchoolHoliday.objects.filter(
                holiday_from__lte=date_range_by_month['first_day'], holiday_till__gte=date_range_by_month['first_day'], academic_calender=request_data['academic_calendar']).count()
            print("####", school_holiday_count)
            working_days = total_no_of_days - school_holiday_count
        elif request_data['calendar_type'] == 'section-calendar':
            acadamic_session_qs = AcademicSession.objects.filter(
                grade=request_data['grade'], section=request_data['section'])
            school_holiday_count = SchoolHoliday.objects.filter(
                holiday_from__lte=date_range_by_month['first_day'], holiday_till__gte=date_range_by_month['first_day'], academic_session=acadamic_session_qs[0].id).count()
            print("####", school_holiday_count)
            working_days = total_no_of_days - school_holiday_count
